...
gRPC リクエスト
     ...
     ▼
┌─────────────────────────────────────────┐
│  protobuf_policy_option.Interceptor     │  .proto の (policy.v1.policy) オプションを読み取り、
...
| モジュール | 責務 |
| --- | --- |
| `protobuf_policy_option` | proto レジストリから `(policy.v1.policy)` メソッドオプションを読み取り、`<placeholder>` トークンをリクエストフィールドで解決し、結果を `context.Context` に格納 |
...
認可が必要なメソッドに `(policy.v1.policy)` オプションを宣言します：

```proto
syntax = "proto3";

import "policy.proto";  // (policy.v1.policy) 拡張を提供
...
`(policy.v1.policy)` オプションのないメソッドは、認可チェックなしでそのまま通過します。
