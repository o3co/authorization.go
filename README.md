...
gRPC request
     ...
     ▼
┌─────────────────────────────────────────┐
│  protobuf_policy_option.Interceptor     │  reads (o3co.authz.v1.policy) option from .proto,
...
| --- | --- |
| `protobuf_policy_option` | Reads the `(o3co.authz.v1.policy)` method option from the proto registry, resolves `<placeholder>` tokens using request fields, and stores the result in `context.Context` |
...
Declare the `(o3co.authz.v1.policy)` option on any method that requires authorization:

```proto
syntax = "proto3";

import "policy.proto";  // provides the (o3co.authz.v1.policy) extension
...
Methods without a `(o3co.authz.v1.policy)` option are passed through without any authorization check.
...
Rules are evaluated locally — no external service needed. Supports exact match, `*` (match all), and prefix wildcards (`posts/*`). Useful for development, simple deployments, or as a starting point before introducing OPA or Cedar.
